	...
	public class Helper
	{
				...
				using (var cmd = conn.CreateCommand())
				{
					...
					using (var reader = cmd.ExecuteReader())
					{
						while (reader.Read())
						{
							...
							var maxByteLength = reader.GetSqlInt16(1).Value;
		...
		public List<List<ProcedureResultSetColumn>> GetProcedureResultSetColumnCollection2005(string objectSchemaName, string objectName)
		{
			...
			try
			{
				...
				var conn = sb.IntegratedSecurity ? new ServerConnection(sb.DataSource) : new ServerConnection(sb.DataSource, sb.UserID, sb.Password);
			    ...
			    retValue.AddRange(from DataTable dt in ds[1].Tables select (from DataColumn column in dt.Columns select new ProcedureResultSetColumn(column.ColumnName, column.DataType.FullName)).ToList());
			}
			catch
			{
		...
		public List<List<ProcedureResultSetColumn>> GetProcedureResultSetColumnCollection2014(string objectSchemaName, string objectName)
		{
		    ...
		    var retValue = new List<List<ProcedureResultSetColumn>> {firstRecordset};
		    return retValue;
		}
